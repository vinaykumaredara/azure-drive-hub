...
serve(async (req) => {
  ...
  try {
    ...
    const { carId, startAt, endAt, licenseUrl, paymentChoice } = await req.json();

    if (!carId || !startAt || !endAt || !licenseUrl || !paymentChoice) {
    ...
    const { data, error } = await supabaseClient.rpc('create_booking_transaction', {
      ...
      p_total_amount: 0, // This will be calculated in the transaction function
      p_pay_mode: paymentChoice,
      p_license_path: licenseUrl
    });

    if (error) throw error;
    if (!data?.success) throw new Error(data?.error || "Failed to create booking");

    console.log(`Created ${paymentChoice} booking ${data.bookingId} for user ${user.id}`);
