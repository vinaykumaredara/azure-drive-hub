...
const CarCardModernComponent = ({
    ...
    try {
      ...
      if (profileLoading) {
        toast({
          ...
          description:
            'Please wait a second while we finish loading your profile...',
        });
        return;
      }
      ...
      toast({
        ...
        description:
          'An unexpected error occurred. Please check the console or contact support.',
        variant: 'destructive',
      });
    }
  }
  ...
  const handleWhatsAppContact = useCallback(() => {
    const text = encodeURIComponent(
      `Hello RP cars, I'm interested in ${car.make} ${car.model} (${car.id})`
    );
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <div className="mt-2 sm:mt-3 flex flex-wrap gap-1 sm:gap-2 text-[0.6rem] sm:text-xs">
              <span className="px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full bg-muted">
                Seats: {car.seats}
              </span>
              <span className="px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full bg-muted capitalize">
                {car.fuel}
              </span>
              <span className="px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full bg-muted capitalize">
                {car.transmission}
